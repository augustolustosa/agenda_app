...
class ContactHelper{
  ...
  Future<int> updateContacts(Contact contato) async{
    Database dbContact = await db;
    return await dbContact.update(contactTable, contato.toMap(), where: "idColumn = ?", whereArgs: [contato.id]);
  }

  Future<List> getAllContacts() async{
    Database dbContact = await db;
    List listMap = await dbContact.rawQuery("SELECT * FROM $contactTable");
    List<Contact> listaMapas = List();
    for(Map m in listMap){
      listaMapas.add(Contact.fromMap(m));
    }
    return listaMapas;
  }
  ...
  Future<int> getNumber() async{
    Database dbContact = await db;
    return Sqflite.firstIntValue(await dbContact.rawQuery("SELECT COUNT(*) FROM $dbContact"));
  }
